...
const ManageOrder = () => {
  ...
  useEffect(() => {
    const url = `https://safe-oasis-95294.herokuapp.com/order`;
    fetch(url)
      .then((res) => res.json())
  ...
  const handleStatus = (state, id) => {
    ...
    const url = `https://safe-oasis-95294.herokuapp.com/orders/${id}`;
    fetch(url, {
      method: "PUT",
      headers: {
  ...
  const handledelete = (id) => {
    ...
    if (confirmBox === true) {
      const url = `https://safe-oasis-95294.herokuapp.com/order/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
